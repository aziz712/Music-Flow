...
export default function SongCard({ song, isSelected, onToggleSelect }: SongCardProps) {
    ...
    const handleDownload = async (e: React.MouseEvent) => {
        ...
        try {
            const url = `http://localhost:5000/api/songs/download?url=${encodeURIComponent(song.link)}&title=${encodeURIComponent(song.title)}&artist=${encodeURIComponent(song.artist.name)}`;
    ...
    const handleWatch = async (e: React.MouseEvent) => {
        ...
        setIsResolving(true);
        try {
            const res = await fetch(`http://localhost:5000/api/songs/resolve?title=${encodeURIComponent(song.title)}&artist=${encodeURIComponent(song.artist.name)}`);

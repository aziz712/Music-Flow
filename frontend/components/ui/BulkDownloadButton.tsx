    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const songsPayload = selectedSongs.map(song => ({
                ...
                downloadUrl: `${process.env.NEXT_PUBLIC_API_URL}/songs/download?url=${encodeURIComponent(song.link)}&title=${encodeURIComponent(song.title)}&artist=${encodeURIComponent(song.artist.name)}`
            }));
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/download/bulk`, {
                method: "POST",
